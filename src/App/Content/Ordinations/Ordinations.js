...
class Ordinations extends React.Component {
  ...
  deleteOrdination = (ordId) => {
    ...
    axios.put('https://deployment-isa.herokuapp.com/api/ordination/delete/' + ordId, {
      responseType: 'json'
    })
      .then(response => {
  ...
  getAppointmentsForOrdination = (ordId) => {
    var token = localStorage.getItem('token');
    axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
    axios.get('https://deployment-isa.herokuapp.com/api/appointment/get-ordination-appointments/' + ordId, {
      responseType: 'json'
    })
      .then(response => {
      ...
      .then(() => {
        axios.get("https://deployment-isa.herokuapp.com/api/clinic-admin-clinic/" + this.state.clinicAdmin)
          ...
          .then(() => {
            ...
            axios.get("https://deployment-isa.herokuapp.com/api/ordination/clinic-ordinations/" + this.state.id)
              .then(response => {
                let tmpArray = []
