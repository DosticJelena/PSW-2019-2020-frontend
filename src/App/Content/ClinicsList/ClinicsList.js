...
class ClinicsList extends React.Component{
    ...
    FilterClinics = event => {
      ...
      axios.get("https://deployment-isa.herokuapp.com/api/filter-clinics/"+this.state.date+'/'+this.state.type).then(response => {
    ...
    componentDidMount() {
      var token = localStorage.getItem('token');
      axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
      axios.get("https://deployment-isa.herokuapp.com/api/clinics").then(response => {
        let tmpArray = []
        for (var i = 0; i < response.data.length; i++) {
            tmpArray.push(response.data[i])
        }
    ...
    var token = localStorage.getItem('token');
    axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
    axios.get("https://deployment-isa.herokuapp.com/api/types")
      .then(response => {
        let tmpArray = []
        for (var i = 0; i < response.data.length; i++) {
          tmpArray.push(response.data[i])
